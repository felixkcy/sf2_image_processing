function Z = jpegdec_dwt(vlc, qstep, n, M, bits, huffval, dcbits, W, H)
  ...
  if (nargin<7)
    ...
    if (nargin<6)
      ...
      if (nargin<4)
        ...
        if (mod(M, 2^n)~=0) error('M must be an integer multiple of 2^n'); end
      end
    end
  end
end
...
% End-of-block detected, save block.
    ...
    % Possibly regroup yq
    if (M > 2^n) yq = regroup(yq, M/2^n); end
    Zq(r+t,c+t) = yq;
  end
end

fprintf(1, 'Inverse quantising\n');
Zi = dwtgroup(quant2dwt(Zq, n, qstep), -n);
