function [Yq, H, Hmat] = quant2dwt(Y, N, dwtstep)
H = 0;
Hmat = zeros(3, N+1);
Yq = Y;
m = length(Y);
for i=1:N
    ...
    for k=1:3
        ...
        Yq(ind_r(k,:), ind_c(k,:)) = quant2(Yq(ind_r(k,:), ind_c(k,:)), dwtstep(k, i), dwtstep(k, i));
        Hmat(k, i) = bpp(Yq(ind_r(k,:), ind_c(k,:))) * m^2;
    end
end
% quantisation of the final low-pass image
Yq(1:m, 1:m) = quant2(Yq(1:m, 1:m), dwtstep(1, N+1), dwtstep(1, N+1));
